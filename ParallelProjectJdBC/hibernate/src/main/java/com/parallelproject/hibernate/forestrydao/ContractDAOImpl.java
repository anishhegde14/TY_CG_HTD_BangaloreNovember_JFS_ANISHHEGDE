...
public class ContractDAOImpl implements ContractorDAO {
	...
	public boolean updateContract(ContractBean bean) {
		...
		try{
			...
			etTransaction.begin();
			String jpql="update ContractBean set qty=:qty,prodid=prodid:,haulid=:haulid where contractno=:contract_no";
			Query q=emManager.createQuery(jpql);
			q.setParameter("qty", bean.getQty());
			q.setParameter("prodid", bean.getProdid());
			q.setParameter("haulid", bean.getHaulid());
